...
export default function Details({ navigation, route }) {
  ...
  const [form, setForm] = useState({ side: '', temperature: '' });
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#F4EFF3' }}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        <Image
          ...
          source={{
            uri:
              dish?.image ||
              'https://images.unsplash.com/photo-1513185158878-8d8c2a2a3da3?q=80&w=1200&auto=format&fit=crop',
          }}
          style={styles.hero}
        />

        <View style={styles.section}>
          <Text style={styles.title}>{dish?.title || 'Dish'}</Text>
          <Text style={styles.subtitle}>
            Pick options below and add to your order.
          </Text>
        </View>
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    {price && <Text style={styles.radioPrice}>+${price}</Text>}
                  </View>
                </TouchableOpacity>
              );
            })}
          ...
          <View style={styles.sectionOptions}>
            {temperatures.map((label, index) => {
              ...
              return (
                <TouchableOpacity
                  key={index}
                  onPress={() => setForm({ ...form, temperature: label })}
                >
                  <View
                    style={[styles.radio, index === 0 && { borderTopWidth: 0 }]}
                  >
      ...
      <View style={styles.overlay}>
        <TouchableOpacity style={{ flex: 1, paddingHorizontal: 24 }}>
          <View style={styles.btn}>
            <Text style={styles.btnText}>
              Add to order (${(dish?.price || 15.95).toFixed(2)})
            </Text>
          </View>
        </TouchableOpacity>
      </View>
...
const styles = StyleSheet.create({
  ...
  title: { fontSize: 27, fontWeight: '700', color: '#1d1d1d', marginBottom: 4 },
  subtitle: { fontSize: 14, lineHeight: 20, color: '#494949' },
  overlay: {
    position: 'absolute',
    bottom: 0,
    left: 0,
  ...
  sectionTitle: { fontSize: 17, fontWeight: '600', color: '#1d1d1d' },
  sectionSubtitle: { fontSize: 13, color: '#6d6d6d' },
  sectionBadge: {
